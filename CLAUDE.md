...
- `src/css/variables.less` — theme colors and other Less variables, including `@linkColor` / `@linkHoverColor` (the hyperlink blue) and the derived accent colors: `@publishedAccentColor` / `@unpublishedAccentColor` for projects. Snippets use a fixed off-white accent (`@textColor`), the same value as `@publishedAccentColor`.
